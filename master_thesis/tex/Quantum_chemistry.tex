...
The first postulate states that everything known about a physical system
...
A system state vector $\ket{\Psi}$ that is not an eigenstate
...
invited to look them up themselves in \cite{Atkins:2011, Cohen:1973}.
...
can measure is $o_i$, which is $ \ket{i} $ using the conventions stated in postulate 3 and 4.
This is because before measuring, the probabilities for any eigenvalue
is as stated in the fourth postulate. When the value has been measured
...
This shows that, since the left-hand side of the equation is only dependent on $x$ and the right-hand
side is only dependent on $t$, no matter how much we change each of the coordinates, they must always equal to
...
To calculate the total energy and wave functions of the system we define a time independent \SE where the wave function is dependent on
...
The Hamiltonian is therefore dependent on the positions of all the electrons and all
the nuclei \cite{Jensen:2017}
\begin{equation}
  \hat{H}(\rvec, \vec{R}) = \hat{T}_e + \hat{T}_N + \hat{V}_{Ne} +\hat{V}_{NN} + \hat{V}_{ee}
\end{equation}
Both repulsion potentials $\hat{V}_{NN}, \hat{V}_{ee}$ have $\frac{N(N - 1)}{2}$ and $\frac{n(n - 1)}{2}$ terms
...
than an electron \cite{NIST:2019}) the electrons can instantaneously respond to
...
\cite{Atkins:2014, Cramer:2004, Jensen:2017}.
...
compute the nuclear repulsion $V_{NN}$. As we are solving for any given geometry
 $V_{NN}$ becomes a constant \cite{Cramer:2004}. The electron
...
\end{equation}
Solving the electronic \ac{SE} for all possible nuclear geometries will give
...
In the following sections we will be talking mostly about solving the electronic \ac{SE},
 where we will drop the subscript and simply denote the electronic
...
\end{equation}
For ease of notation we assume that  $\Phi$ is normalized. This the
following
\begin{equation}
  \braket{\Phi|\Phi} = \sum\limits_i^nc_i^2 = 1
\end{equation}
 Additionally, the energy associated with the Hamiltonian can
be calculated as in Equation \ref{eq:endevaleq} to give us
\begin{equation}
  \braket{\Phi|\hat{H}|\Phi} = \sum\limits_i^nc_i^2E_i\label{eq:genenerg}
\end{equation}
This tells us the energy of the wave function $\Phi$ can be determined by
...
Subtracting $E_0$ from Equation \ref{eq:genenerg} to find the difference
...
\cite{Cramer:2004}, as each individual $E_i$ may add more energy to the
...
\begin{equation}
  ...
  \end{split}
\end{equation}
It can be inferred from \ref{eq:endevaleq} that the inequality in the last term in
...
\ac{SE} for each geometry of the molecule. Secondly we can construct
...
Next, we intend to find a way to systematically create wave functions and
...
The spatial orbitals can, and are mostly, built using a basis set as described in
...
For each electron,  we are accounting for the repulsion and exchange interactions with all the
...
approximation, where we only take into account the lowest energy level of the system, and
...
determines the $V_{ext}$ \cite{Cramer:2004}. The Hamiltonian of the system can thus be defined and
...
The important points from the Hohenberg--Kohn theorems are that the energy of
